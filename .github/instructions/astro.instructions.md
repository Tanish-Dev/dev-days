...
- Document the `Props` interface and each non-obvious property with concise comments so callers understand the component contract.
- Comments should explain intent, constraints, or design decisions. Do not restate markup, class names, or expressions that are self-explanatory.
- Keep comments synchronized with the implementation; stale documentation is a defect.
...
- Use explicit property types and document the `Props` interface for reusable components
